<script lang="ts">
	...
	const pjlPreviewRequest = $derived(shared.pjlPreviewRequest)
	...
	const pjlPreviewForCurrentArticle = $derived.by(() => {
		const currentId = page.url.searchParams.get("article") ?? undefined
		if (!currentId || !pjlPreviewRequest) return undefined
		return pjlPreviewRequest.articleId === currentId
			? pjlPreviewRequest
			: undefined
	})
	$effect(() => {
	...
	const pjlAggregateProjection = $derived.by(() => {
		...
		for (const block of pjlBlocksForArticle) {
			const { directives, isAction } = buildDirectivesFromPjlBlock(
				block,
	...
	const projectedHtmlResult = $derived.by(() => {
		if (!pjlPreviewForCurrentArticle) return undefined
		const directive = findDirectiveForPreviewRequest(
			pjlPreviewForCurrentArticle,
			articleInfo.article?.num,
		)
		if (!directive) {
			return {
				html: null,
				reason:
					"Disposition ciblée introuvable parmi les actions projetables de ce bloc.",
			} satisfies ProjectedHtmlResult
		}
		const baseHtml = isSectionRequest ? "" : (currentBlocTextuel ?? "")
		return applyProjectActionsToHtml(baseHtml, [directive])
	})
	...
	const pjlDiffContent = $derived.by(() => {
		if (projectedHtmlResult) {
			if (projectedHtmlResult.html === null) {
				return `<div class="font-sans text-sm text-le-gris-dispositif-dark py-4 text-center ">${projectedHtmlResult.reason}</div>`
			}
			if (
				pjlPreviewForCurrentArticle?.mode === "single_action_diff" &&
				currentBlocTextuel &&
				projectedHtmlResult.skipDiff !== true
			) {
				...
				{#if pjlBlocksForArticle.length > 0}
					...
					>
						...
						<div class="mt-3 flex items-center justify-end">
							...
							>
								{showPjlDiff && !pjlPreviewForCurrentArticle
